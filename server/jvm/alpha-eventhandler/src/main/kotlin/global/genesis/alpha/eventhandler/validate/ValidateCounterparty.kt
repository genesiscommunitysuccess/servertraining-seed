package global.genesis.alpha.eventhandler.validate
...
class ValidateCounterparty {
    companion object {
        suspend fun validateDelete(
            ...
            val trade = entityDb.getRange(Trade.byCounterpartyId(counterpartyId)).toList().firstOrNull()
            val counterparty = entityDb.get(Counterparty.byId(counterpartyId))

            require(counterparty != null) { "Counterparty not found" }
            require(trade == null) { "Trades are using this Counterparty. Unable to delete" }
